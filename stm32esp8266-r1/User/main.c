...
void TIM2_IRQHandler(void)
{
	...
	//	POWER = BV*SC; 
		...
		Delay_s(2);
		printf("{\"sensor_tag\":\"temperature1\",\"value\":\"%d\"}\r\n", temp);	//发送数据字节数
		Delay_s(1);
		
		printf("AT+CIPSEND=39\r\n");	//发送数据字节数
		Delay_s(2);
		printf("{\"sensor_tag\":\"humidity1\",\"value\":\"%d\"}\r\n", humi);	//发送数据字节数
		Delay_s(1);
		
		printf("AT+CIPSEND=41\r\n");	//发送数据字节数
		Delay_s(2);
		printf("{\"sensor_tag\":\"voltage1\",\"value\":\"%.3f\"}\r\n",BVF);	//发送数据字节数
		Delay_s(1);
		
		printf("AT+CIPSEND=41\r\n");	//发送数据字节数
		Delay_s(2);
		printf("{\"sensor_tag\":\"current1\",\"value\":\"%.3f\"}\r\n", SCF);	//发送数据字节数
		Delay_s(1);
		
		printf("AT+CIPSEND=44\r\n");	//发送数据字节数
		Delay_s(2);
		printf("{\"sensor_tag\":\"illuminance1\",\"value\":\"%04d\"}\r\n",light );	//发送数据字节数
		Delay_s(1);
